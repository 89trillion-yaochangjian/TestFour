...
func CreateGiftCodeService(giftCodeInfo model.GiftCodeInfo) (string, *status.Response) {
	...
	CodeInfo, err := model.CreateGiftCodeDao(code, jsonCodeInfo, validPeriod)
	if err != nil {
		return "", err
	}
	return CodeInfo, err
...
func GetGiftCodeInfoService(code string) (model.GiftCodeInfo, *status.Response) {
	//根据礼品码查询礼品信息
	CodeInfo, err := model.GetGiftCodeInfoDao(code)
	if err != nil {
		return CodeInfo, err
	}
	//显示礼包类型
...
func VerifyFiftCodeService(code string, Uid string) ([]byte, error) {
	...
	//获取礼包码信息
	CodeInfo, GteInfoRrr := model.GetGiftCodeInfoDao(code)
	if GteInfoRrr != nil {
		err := errors.New("礼包码无效")
		return nil, err
	}
	userInfo, _ := model.FindUser(Uid)
	...
	case -1:
		...
		Reward1, _ := model.VerifyFiftCodeDao(CodeInfo, userInfo, Uid)
		return proto.Marshal(&Reward1)
	case 0:
		if CodeInfo.AvailableTimes > CodeInfo.ReceiveNum {
			Reward2, _ := model.VerifyFiftCodeDao(CodeInfo, userInfo, Uid)
